  ...
  try {
    switch (action) {
      ...
      case 'tours/book': {
        ...
        return json({ success: true, message: 'Tour booked successfully', total: total_amount });
      }
      ...
      case 'bookings/create': {
        ...
        const r = await db.execute({
          ...
          args: [1, vehicle_id, ref, date + ' ' + time + ':00', (input.return_date || date) + ' ' + (input.return_time || time) + ':00', pickup, input.pickup_lat || null, input.pickup_lng || null, drop, input.drop_lat || null, input.drop_lng || null, distance, duration_days, base_fare, tax, discount, total, trip_type, pricing_type, package_id]
        });
        await db.execute({ sql: "UPDATE vehicles SET total_bookings = total_bookings + 1 WHERE id=?", args: [vehicle_id] });
      ...
      case 'sos/create': {
        await db.execute({ sql: "INSERT INTO sos_alerts (booking_id, user_id, alert_type, message, lat, lng) VALUES (?,?,?,?,?,?)", args: [input.booking_id || null, 1, input.alert_type || 'emergency', input.message || '', input.lat || null, input.lng || null] });
        return json({ success: true, message: 'SOS alert sent to admin' });
      }

      case 'notifications/list': {
        const r = await db.execute({ sql: "SELECT * FROM notifications WHERE user_id=? ORDER BY created_at DESC LIMIT 50", args: [1] });
        return json(r.rows);
      }
      ...
      case 'notifications/create': {
        ...
        if (!targetUserId || !input.title) return err('Missing user_id or title');
        await db.execute({ sql: "INSERT INTO notifications (user_id, title, message, type) VALUES (?,?,?,?)", args: [targetUserId, input.title, input.message || '', input.type || 'info'] });
        return json({ success: true });
      }
